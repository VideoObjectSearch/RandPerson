...
def pairwise_distance(prob_fea, gal_fea):
    with torch.no_grad():
        gal_fea = F.normalize(gal_fea.cuda()).t()
        prob_fea = F.normalize(prob_fea.cuda())
        score = prob_fea.mm(gal_fea)
    return 1. - score.cpu()
...
def evaluate_all(distmat, query=None, gallery=None,
    ...
    for k in cmc_topk:
        print('  top-{:<4}{:12.1%}'
              .format(k, cmc_scores['market1501'][0][k - 1]))

    return cmc_scores['market1501'][0][0], mAP, cmc_scores['market1501'][1]
...
class Evaluator(object):
    ...
    def evaluate(self, test_name, query_loader, gallery_loader, testset, output_dir,
                 tau=100, sigma=100, K=100, alpha=0.1):
        query = testset.query
        gallery = testset.gallery
        prob_fea, problist = extract_features(self.model, query_loader)
        prob_fea = torch.cat([prob_fea[f].unsqueeze(0) for f, _, _, _ in query], 0)
        gal_fea, gallist = extract_features(self.model, gallery_loader)
        gal_fea = torch.cat([gal_fea[f].unsqueeze(0) for f, _, _, _ in gallery], 0)
        galleryList = []
        probeList = []
        for gal in gallist.items():
            galleryList.append(gal[0])
        for pro in problist.items():
            probeList.append(pro[0])
        print('Compute similarity...', end='\t')
        start = time.time()
        dist = pairwise_distance(prob_fea, gal_fea)
        print('Time: %.0f seconds.' % (time.time() - start))
        rank1, mAP, matches = evaluate_all(dist, query=query, gallery=gallery)
       # f = open(sys.path[0].replace('Source/ablation',"")+ output_dir.replace('../../','') + '/'+test_name+'.txt','a')
        #for i in range(matches.shape[0]):
        #    for j in range(i, matches.shape[1]):
        #        if not matches[i][j]:
         #           f.write(galleryList[j]+','+probeList[i]+'\n')
        return rank1, mAP
        ...
        #     dist_rerank[:num_prob, :num_prob] = pairwise_distance(prob_fea, prob_fea)
        #     dist_rerank[num_prob:, num_prob:] = pairwise_distance(gal_fea, gal_fea)
        #
        # print('Time: %.0f seconds' % (time.time() - start))
