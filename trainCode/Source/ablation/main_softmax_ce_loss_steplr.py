...
from evaluators_cos import Evaluator
...
def get_test_data(dataname, data_dir, height, width, test_batch=2048):
    ...
    train_loader = DataLoader(
        Preprocessor(dataset.train,
                     root=osp.join(dataset.images_dir, dataset.train_path), transform=test_transformer),
        batch_size=test_batch, num_workers=32,
        shuffle=False, pin_memory=True)
...
def main(args):
    ...
    # Create model
    model = resgap.create(args.arch, final_layer=args.final_layer, neck=args.neck).cuda()
    ...
    # Criterion
    criterion = SoftmaxCELoss(num_classes, num_features, args.dropout).cuda()
    ...
    if not args.evaluate:
        ...
        for epoch in range(start_epoch, args.epochs):
            loss, acc = trainer.train(epoch, args.epochs, output_dir, train_loader, optimizer, args.print_freq)
    ...
    for test_name in test_names:
        ...
        test_rank1, test_mAP = \
            evaluator.evaluate(test_name, test_query_loader, test_gallery_loader, testset, output_dir)
        ...
        #     evaluator.evaluate(test_query_loader, test_gallery_loader, testset)
        #
...
if __name__ == '__main__':
    ...
    # training configs
    parser.add_argument('--resume', type=str, default='', metavar='PATH', help="'', 'ori' or a real path")
    ...
    parser.add_argument('--method', type=str, default='SoftmaxCELoss')
