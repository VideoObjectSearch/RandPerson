...
def pairwise_distance(prob_fea, gal_fea, loss_layer, ker_batch_size=1000, prob_batch_size=1000):
    with torch.no_grad():
        qaconv = torch.nn.DataParallel(QAConv(gal_fea.cuda(), loss_layer, ker_batch_size, prob_batch_size).cuda()).cuda().eval()
        prob_fea = prob_fea.cuda()
        score = qaconv(prob_fea)
    return 1. - score.cpu()
...
class Evaluator(object):
    ...
    def evaluate(self, query_loader, gallery_loader, testset, loss_layer, ker_batch_size=4096, prob_batch_size=8,
                 tau=100, sigma=100, K=100, alpha=0.1):
        query = testset.query
        ...
        start = time.time()
        dist_t = pairwise_distance(gal_fea, prob_fea, loss_layer, ker_batch_size, prob_batch_size)  # switch gal and prob to have a small kernel
        print('Time: %.0f seconds.' % (time.time() - start))
        dist = dist_t.t()
        ...
        with torch.no_grad():
            ...
            dist_rerank[:num_prob, :num_prob] = pairwise_distance(prob_fea, prob_fea, loss_layer, ker_batch_size, prob_batch_size)
            # dist_rerank[num_prob:, num_prob:] = pairwise_distance(gal_fea, gal_fea, loss_layer, ker_batch_size, prob_batch_size)  # out of memory
            gal_batch_size = num_prob
            for i in range(0, num_gal, gal_batch_size):
                i_end = min(num_gal, i + gal_batch_size)
                dist_rerank[num_prob:, num_prob + i: num_prob + i_end] = \
                    pairwise_distance(gal_fea, gal_fea[i: i_end], loss_layer, ker_batch_size, prob_batch_size)
